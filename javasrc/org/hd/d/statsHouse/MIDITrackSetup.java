...
public record TrackSetup(byte channel, byte instrument, byte volume, byte pan)
	{
    public TrackSetup
	    {
    ...
    public TrackSetup(final byte channel, final byte instrument, final byte volume)
    	{ this(channel, instrument, volume, DEFAULT_PAN); }
    /**The volume is set to its default and pan is set to the default centre position. */
    public TrackSetup(final byte channel, final byte instrument)
