...
#
# NEW IN V4.6:
#    * Slightly fancier playout-note logic.
#    * Start of randomisation in percussion when allowed.
#
# NEW IN V4.5:
...
# Set a seed n>0, -1 means dynamic/new each time, 0 means none or data-derived.
TSEED=0
...
gawk -F, </dev/null \
    ...
    -e 'BEGIN {
         ...
         # If dynamic seed (<0) is requested, create it for this run.
         if(TSEED < 0) { srand(); tseed = int(65536 * rand()); }
...
# Lowest channel used for data melody is 1.
...
print ti ", 0, Control_c, "ci", 10, "pan;
            ...
            # Very last note played on this channel, or an octave below root.
            LASTNOTE = OFFSET - 12;
            ...
            while((getline < SRCCSV) > 0) {
                ...
                # Extract the final part of the date, eg month or day.
                nDateParts = split($1, a, "-");
                lastDatePart = a[nDateParts];
...
print ti ", "clocks", Note_on_c, "ci", "AVENOTE", "int(vol);
                ...
                # Play note where data is present (and in range).
                dataRaw = $(1 + 3*i);
                coverageRaw = $(0 + 3*i);
                ...
                # Remember last note actually voiced.
                LASTNOTE = NOTE;
                # Set the velocity based on being a primary voice or not.
...
print ti ", "(clocks+NoteDeltaTime-1)", Note_off_c, "ci", "NOTE", 0";
            ...
            if(fancyMelody) {
                # Before ending track, if doing 'fancy' melody,
                # play out holding note quietly to end of phrase if needed.
                # Use final note for non-primary, else octave below 'base' note.
                ...
                if(doingAlign && (endOfPhrase > clocks)) {
                    PLAYOUTNOTE = OFFSET - 12; # Playout root down 1o.
                    # Instead last note down 1o if in normal range.
                    if(LASTNOTE >= OFFSET) { PLAYOUTNOTE = LASTNOTE - 12; }
print ti ", "clocks", Note_on_c, "ci", "PLAYOUTNOTE", 1";
print ti ", "endOfPhrase", Note_off_c, "ci", "PLAYOUTNOTE", 0";
